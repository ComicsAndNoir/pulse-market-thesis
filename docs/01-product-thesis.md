# 01 · Product thesis, rubric & metrics

## The bet

**Point Pulse at Exit / Churn Interviews first.** It is the fastest path to proof: the
...
Weights encode what actually kills a 0–1 product: you can't reach the buyer, or you can't
prove value before the room loses patience. So proof-speed and ROI are weighted highest,
...
## The segments (candidate conversations)
...
## The kill list is the headline moment

Naming what Pulse is **not** — unprompted — is the "Kill Discipline" competency made
visible, and almost no candidate does it. **Weekly 1:1s is the loud kill:** the biggest,
most obvious market is exactly the trap, because there is no single buyer and no dollar to
point at. Retiring it on purpose, and saying precisely why, is the point.

## The winner's ROI skeleton (first-principles, illustrative)

- **Buyer:** Head of Retention / Customer Experience — someone measured on a churn or NRR number.
...
- **What would have to be true:** you can source ~50 recently-churned customers and interview them inside two weeks; the interview surfaces reasons a survey misses; the buyer owns the number.

## Competency mapping (say these if probed)

- **Market Discovery** → the segment set + framing every score as a hypothesis to validate.
- **Ruthless Prioritization** → the weighting scheme and the single-winner output.
- **Kill Discipline** → the explicit kill list; the 1:1s kill is the headline.
- **AI-First Operator** → built a working interactive tool fast with AI tooling.
- **Buyer Proximity** → the ROI skeleton framed around a named buyer who owns a number.
- **Owner's Bias** → walked in with a thesis + sequenced roadmap for a product not yet owned.
